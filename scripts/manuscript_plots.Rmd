...
## Number of observations per participants

```{r}
data_RTs_cens_NA_removed %>%
  dplyr::group_by(ppid, TLC_failure) %>%
  dplyr::summarise(n = n()) %>%
  View()
```
...
subset_sample_plot <- ggplot() +
  ...
  geom_jitter(data_RTs_cens_NA_removed %>%
                dplyr::filter(ppid == 5 | ppid == 8 | ppid == 11), mapping = aes(x = TLC_failure, TLC_takeover, col = as.factor(ppid)), seed = 1, width = 0.5, alpha = .6, size = 1.5) +
  xlab(expression(TLC[F] ~ "(s)")) +
  ylab(expression(TLC[T] ~ "(s)")) +
  ggtitle("B") +
  ylim(0, 7) +
  xlim(0, 11) +
  theme_bw() +
  theme(legend.position = "none", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_rect(fill = "transparent"), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 15), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.text = element_text(face = "bold", size = 10))
...
fig_2 <- ggplot() +
  ...
  geom_point(data_RTs_cens_NA_removed %>%
               dplyr::filter(ppid == 5 | ppid == 8 | ppid == 11), mapping = aes(x = TLC_failure, TLC_takeover, col = as.factor(ppid)), position = position_jitter(seed = 42, width = 0.3), alpha = .6, size = 1.5) +
  geom_line(mod_2_predict_ppids %>%
              dplyr::filter(ppid == 5 | ppid == 8 | ppid == 11), mapping = aes(x = TLC_failure, y_hat, col = as.factor(ppid))) +
  xlab(expression(TLC[F] ~ "(s)")) +
  ylab(expression(TLC[T] ~ "(s)")) +
  ylim(0, 7) +
  xlim(0, 11) +
  theme_bw() +
  theme(legend.position = "none", legend.direction = "horizontal", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_rect(fill = "transparent"), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 15), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.text = element_text(face = "bold", size = 10))

ggsave(here::here("multilevel_model_HF_applications/plots/fig_2.tiff"), plot = fig_2, width = 8, height = 7, units = 'cm', dpi = 300, type = 'cairo')
```
...
fig3 <- ggplot() +
  geom_point(data_RTs_cens_NA_removed %>%
               dplyr::filter(ppid == 5 | ppid == 8 | ppid == 11), mapping = aes(x = TLC_failure, TLC_takeover, col = as.factor(ppid)), position = position_jitter(seed = 42, width = 0.3), alpha = .8, size = 1.5) +
  geom_line(mod_2_predict_ppids %>%
              dplyr::filter(ppid == 5 | ppid == 8 | ppid == 11), mapping = aes(x = TLC_failure, y_hat, col = as.factor(ppid))) +
  xlab(expression(TLC[F] ~ "(s)")) +
  ylab(expression(TLC[T] ~ "(s)")) +
  ...
  xlim(0, 11) +
  theme_bw() +
  theme(legend.position = "none", legend.direction = "horizontal", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_rect(fill = "transparent"), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 15), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.background = element_blank(), strip.text.x = element_blank())

ggsave(here::here("multilevel_model_HF_applications/plots/fig_3.tiff"), plot = fig3, width = 16, height = 7, units = 'cm', dpi = 300, type = 'cairo')
```
...
ggsave(here::here("multilevel_model_HF_applications/plots/fig_4.tiff"), plot = fig_4, width = 8, height = 7, units = 'cm', dpi = 300, type = 'cairo')
```
...
```{r}
slopes <- coef(mod_4)$ppid %>%
  rownames_to_column("ppid") %>%
  select(ppid, n_backTRUE)

se <- ranef(mod_4)$ppid %>%
  as.data.frame() %>%
  tibble::rownames_to_column("ppid") %>%
  rename(se_n_back = n_backTRUE)

plot_dat <- left_join(slopes, se, by = "ppid")

plot_dat <- plot_dat %>%
  dplyr::mutate(
    lower = n_backTRUE - 1.96 * se_n_back,
    upper = n_backTRUE + 1.96 * se_n_back
  )

fig5a <- ggplot(plot_dat, aes(x = reorder(ppid, n_backTRUE), y = n_backTRUE)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0) +
  coord_flip() +
  ggtitle("A") +
  xlab("Participant") +
  ylab(expression(paste("Participant slopes (", beta[N[j]], ")"))) +
  theme_bw() +
  theme(legend.position = "none", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_blank(), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 8), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.text = element_text(face = "bold", size = 10))


mod_4_ranef <- as.data.frame(coef(mod_4)$ppid) %>%
  dplyr::rename_all(~c("Intercepts", "n_back_effect", "lead_vehicle_effect", "interaction_effect")) %>%
  dplyr::mutate("Participant" = row_number())

# correlations between random slopes and random intercepts
fig5b <- ggplot(mod_4_ranef, mapping = aes(x = Intercepts, y = n_back_effect)) +
  geom_point() +
  geom_vline(aes(xintercept = fixef(mod_4)[1]), linetype = "dashed") +
  geom_hline(aes(yintercept = fixef(mod_4)[2]), linetype = "dashed") +
  ggtitle("B") +
  xlim(.2, .8) +
  ylim(-.35, .2) +
  xlab(expression(paste("Participant intercepts (", beta[0[j]], ")"))) +
  ylab(expression(paste("Participant slopes (", beta[N[j]], ")"))) +
  theme_bw() +
  theme(legend.position = "none", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_blank(), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 15), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.text = element_text(face = "bold", size = 10))
...
ggsave(here::here("plots/fig_5.tiff"), plot = fig5, width = 16, height = 12, units = 'cm', dpi = 300, type = 'cairo')
```
...
fig_6 <- ggplot() +
  geom_jitter(individual_effects, mapping = aes(x = x, y = beta_n_j), width = .05, height = 0, size = 4,
  ...
  geom_hline(aes(yintercept = intervals$`2.5 %`[6], linetype = "95% CI"),  size = 1.5) +
  geom_hline(aes(yintercept = intervals$`97.5 %`[6], linetype = "95% CI"),  size = 1.5) +
  geom_hline(aes(yintercept = fixef(mod_4)[2], linetype = "beta_1"), size = 1.5) +
  geom_hline(aes(yintercept = (fixef(mod_4)[2] + 1.96 * sigma_beta_n), linetype = "95% HI"), size = 1.5) +
  geom_hline(aes(yintercept = (fixef(mod_4)[2] - 1.96 * sigma_beta_n), linetype = "95% HI"), size = 1.5) +
  scale_linetype_manual(name = " ", values = c(2, 3, 1), labels = c(expression(CI[95]), expression(HI[95]), expression(beta[N]))) +
  theme_bw() +
...
ggsave(here::here("plots/fig_6.tiff"), plot = fig_6, width = 16, height = 6, units = 'cm', dpi = 300, type = 'cairo')
```

...
```{r}
data_RTs_cens_NA_removed <- fread(file = here::here("multilevel_model_HF_applications/data/data_RTs_cens_NA_removed.csv"))
```
...
fig5a <- ggplot(plot_dat, aes(x = reorder(ppid, n_backTRUE), y = n_backTRUE)) +
  ...
  ggtitle("A") +
  xlab("Participant") +
  ylab(expression(beta[N[j]])) +
  theme_bw() +
  theme(legend.position = "none", legend.key.width = unit(0.1, 'cm'), legend.key.height = unit(0.1, 'cm'), legend.key = element_blank(), axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 6), title = element_text(size = 25), legend.title = element_text(size = 12), legend.text = element_text(size = 12), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 22, face = "bold"), strip.text = element_text(face = "bold", size = 10))
...
ggsave(here::here("plots/fig_5.tiff"), plot = fig5, width = 16, height = 10, units = 'cm', dpi = 300, type = 'cairo')
```
...
fig_6 <- ggplot() +
  ...
  geom_hline(aes(yintercept = intervals$`2.5 %`[6], linetype = "95% CI"),  size = 1.5,  color = viridis(5)[3]) +
  geom_hline(aes(yintercept = intervals$`97.5 %`[6], linetype = "95% CI"),  size = 1.5,  color = viridis(5)[3]) +
  geom_hline(aes(yintercept = fixef(mod_4)[2], linetype = "beta_1"), size = 1.5, color = "black") +
  geom_hline(aes(yintercept = (fixef(mod_4)[2] + 1.96 * sigma_beta_n), linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
  geom_hline(aes(yintercept = (fixef(mod_4)[2] - 1.96 * sigma_beta_n), linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
...
ggsave(here::here("multilevel_model_HF_applications/plots/fig_6.tiff"), plot = fig_6, width = 16, height = 6, units = 'cm', dpi = 300, type = 'cairo')
```
